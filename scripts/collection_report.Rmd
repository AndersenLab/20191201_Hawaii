...
isolation_counts3 <- fulcrum_dat %>%
  dplyr::mutate(collection_type = ifelse(worms_on_sample %in% c("No", "?"), "No Nematode",
                                   ifelse(worms_on_sample == "Tracks", "Tracks only",
                                          ifelse(worms_on_sample == "Yes" & is.na(ITS2_pcr_product), "Not genotyped",
                                                 ifelse(worms_on_sample == "Yes" & ITS2_pcr_product == 0, "PCR -", species_id))))) %>%
...
`Isolations that need to be resequenced` <- fulcrum_dat %>%
  dplyr::filter(ITS2_pcr_product == 1) %>%
...
`duplication in genotyping sheet` <- googlesheets::gs_key("1XcfOLYGZcbXzT_VznDgBSWW10QfIv3eE7YEHOaGpTic") %>%
...
**Duplication issue 1:** Duplication in feild sampling comes from collection_id C-0871.<br>
**Solution:** Remove Gaotian's instance of C-0871 after extracting collection data from it and applying these data to all<br>
collections that contained the `Location at 871` flag in the `substrate_notes` column. See lines 503-523 in<br>
process_fulcrum_data.R script.<br>
**Duplication issue 2:** Duplicated isolation record for S-11690.<br> 
**Solution:** The extra S-11690 record was removed in Step 5 of process_fulcrum_data.R script.<br>
...
### Concerns as of 20200119
**1.** One collection (C-0882) has a c-label (C-0882) rather than an s-label for isolation_id.<br>
**2.** One s-label is S-XXXXX? Could this resolve concern 1?<br>
**3.** Possible new caeno species isolations need to be confirmed.<br>
**4.** There are 28 isolations that have notes suggesting they need to be resequenced. These include all isolations with notes mentioning redos, ITS2 positive without species_ids, or species_ids as "No significance", "No amplification", or "unclutured soil fungus". 